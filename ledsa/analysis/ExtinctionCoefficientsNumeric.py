...
class ExtinctionCoefficientsNumeric(ExtinctionCoefficients):
    """Calculate the extinction coefficients with the fit approach."""
    ...
    def _calc_coefficients_of_img(self, rel_intensities: np.ndarray) -> np.ndarray:
        ...
        fit = minimize(self._cost_function, kappa0, args=rel_intensities,
                       method='TNC', bounds=tuple(self.bounds),
                       options={'maxiter': 200, 'gtol': 1e-5, 'disp': True})
        kappas = np.flip(fit.x)
        return kappas

    def _calc_intensities(self, kappas: np.ndarray) -> np.ndarray:
    ...
    def _cost_function(self, kappas: np.ndarray, target: np.ndarray) -> float:
        intensities = self._calc_intensities(kappas)
