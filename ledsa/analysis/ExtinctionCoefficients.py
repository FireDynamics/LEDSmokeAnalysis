...
class ExtinctionCoefficients(ABC):
    """Abstract base class for the different extinction coefficinet calculation methods."""
    ...
    def calc_and_set_coefficients(self):
        """Calculate all extinction coefficients."""
        self._set_all_member_variables()
        for img_id, single_img_data in self.calculated_img_data.groupby(level=0):
            ...
            kappas = self._calc_coefficients_of_img(rel_intensities)
            self.coefficients_per_image_and_layer.append(kappas)

    def calc_and_set_coefficients_mp(self, cores=4):
        """Multicore implementation of calc_and_set_coefficients."""
        self._set_all_member_variables()
        ...
        kappas = pool.map(self._calc_coefficients_of_img, rel_intensities)
        pool.close()
        self.coefficients_per_image_and_layer = kappas

    def save(self):
        """Save calculated extinction coefficients into a csv file."""
    ...
    def _set_all_member_variables(self):
        if len(self.distances_per_led_and_layer) == 0:
            self.distances_per_led_and_layer = self._calc_distance_array()
        if self.calculated_img_data.empty:
            self._load_img_data()
        if self.ref_intensities.shape[0] == 0:
            self._calc_and_set_ref_intensities()
    ...
    def _calc_distance_array(self) -> np.ndarray:
    ...
    def _calc_and_set_ref_intensities(self) -> None:
