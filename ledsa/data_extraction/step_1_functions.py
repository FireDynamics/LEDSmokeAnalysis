...
def find_search_areas(image: np.ndarray, window_radius, pixel_value_percentile=99.875, max_n_leds=1300) -> np.ndarray:
    """
    ...
    :param window_radius: The radius of the square area around each identified LED location.
    ...
    while max_pixel_value > threshold and led_id < max_n_leds:
        ...
        if max_pixel_value > threshold:
            image[max_pixel_loc[1] - window_radius: max_pixel_loc[1] + window_radius, max_pixel_loc[0] - window_radius: max_pixel_loc[0] + window_radius] = 0
...
def add_search_areas_to_plot(search_areas: np.ndarray, window_radius: int, ax: plt.axes) -> None:
    """
    ...
    :param window_radius: The radius of the search areas to be displayed
    :type window_radius: int
    ...
    for i in range(search_areas.shape[0]):
        ax.add_patch(plt.Circle((search_areas[i, 2], search_areas[i, 1]),
                                ...
                                linewidth=0.1))
        ax.text(search_areas[i, 2] + int(window_radius),
                search_areas[i, 1] + int(window_radius) // 2,
