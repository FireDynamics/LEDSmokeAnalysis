...
def generate_led_analysis_data(conf, channel, data, debug, iled, img_filename, led_array_idx, search_areas,
    ...
    img_data = generate_led_analysis_data_string(center_sa_x, center_sa_y, fit_res, iled, led_array_idx,
                                                 mean_color_value,
                                                 sum_color_value, window_radius, fit_time)

    if not fit_res.success:  # A > 255 or A < 0:
        log_warnings(img_filename, channel, fit_res, iled, led_array_idx, data, search_area, center_sa_x, center_sa_y, conf)
    return img_data
...
def save_results_in_file(channel, img_data, img_filename, img_id, img_infos, root):
    ...
    out_file.write("   fit_fun, fit_nfev , fit_time, sum_col_value, average_col_value, "
                   "// all spatial quantities in pixel coordinates\n")
...
def generate_led_analysis_data_string(cx, cy, fit_res, iled, led_array_idx, mean_color_value, sum_color_value,
    ...
    led_data = (f'{iled:4d},{led_array_idx:2d},{im_x:10.4e},{im_y:10.4e},{dx:10.4e},{dy:10.4e},{A:10.4e},'
                ...
                f'{fit_res.nfev:9d},{fit_time:10.4e},{sum_color_value:10.4e},{mean_color_value:10.4e}\n')
    return led_data


def log_warnings(img_filename, channel, fit_res, iled, line_number, data, s, cx, cy, conf):
