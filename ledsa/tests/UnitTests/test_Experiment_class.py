...
class HeightMethodTestCase(TestExperiment):
    def test_led_height_eq_camera_height(self):
        ...
        th = self.experiment._calc_traversed_height_in_layer(led_height, 0, 2)
        self.assertAlmostEqual(th, 0)

    def test_led_and_camera_on_same_side(self):
        led_height = 0.5
        th = self.experiment._calc_traversed_height_in_layer(led_height, 2, 2.5)
        self.assertAlmostEqual(th, 0)

    def test_led_and_camera_on_different_sides(self):
        led_height = 3
        th = self.experiment._calc_traversed_height_in_layer(led_height, 1.5, 2.5)
        self.assertAlmostEqual(th, 1)

    def test_camera_inside_led_outside(self):
        led_height = 2
        th = self.experiment._calc_traversed_height_in_layer(led_height, 0.5, 1.5)
...
class DistConsistencyMethodTestCase(TestExperiment):
    def test_same_distances(self):
        ...
        self.assertTrue(self.experiment._distance_calculation_is_consistent(dist_array, self.experiment.leds[0]))

    def test_different_distances(self):
        dist_array = np.array([0, 1, 2, 1, 1, 0])
        self.assertFalse(self.experiment._distance_calculation_is_consistent(dist_array, self.experiment.leds[0]))
...
class CalcTraversedDistTestCase(TestExperiment):
    def test_calc_traversed_dist_per_layer(self):
        ...
        self.assertTrue(self.experiment._distance_calculation_is_consistent(dists, self.experiment.leds[4]))

    def test_calc_traversed_dist_in_plane(self):
        dists = self.experiment._calc_traversed_dist_in_plane(LED(0, 5, 3, 1))
...
class GetLEDPositions(TestExperiment):
    ...
    def test_file_not_formatted_right(self, mock_loadtxt):
        self.assertRaises(IndexError, lambda: self.experiment._get_led_positions(np.array([0, 1, 2])))
    ...
    def test_file_exists_and_works(self, mock_loadtxt):
        led_x, led_y, led_z = self.experiment._get_led_positions(np.array([0, 1, 2]))
...
class SetLEDs(TestExperiment):
    def setUp(self):
        super().setUp()
        self.experiment._get_led_ids = MagicMock(return_value=[0, 1, 2, 3, 4])
        self.experiment._get_led_positions = MagicMock(return_value=[np.array([0, 1, 2, 3, 4]),
                                                                     np.array([3, 3, 3, 3, 3]),
                                                                     np.array([0.5, 1, 1.5, 2, 2.5])])

    def test_leds_set_right(self):
        self.experiment._set_leds()
